...
<head>
  ...
  <style>
    :root{
      ...
      --pill-bg: #ff63e6;     /* purple pill */
      --pill-border: #ff9fe6;
      font-family: Inter, system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial;
    }
    *{box-sizing:border-box}
    html,body{height:100%;margin:0;background:linear-gradient(180deg,#eef2ff 0%,var(--bg) 100%);color:#111}

    /* Header */
    .header{
      width:100%;padding:14px 22px;display:flex;justify-content:space-between;
      align-items:center;background:var(--card);box-shadow:0 4px 18px rgba(16,24,40,0.06);
      position:sticky;top:0;z-index:100;border-radius:0 0 12px 12px;margin-bottom:18px;
    }
    .welcome{font-size:18px;font-weight:600}
    .profile{
      width:44px;height:44px;border-radius:50%;background:#dbeafe;display:flex;
      justify-content:center;align-items:center;font-weight:700;color:var(--accent);font-size:18px;
    ...
    input[type=text]{padding:10px;border-radius:8px;border:1px solid #e6e9ee;font-size:14px}
    ...
    .btn.secondary{background:#e6eefc;color:var(--accent);font-weight:600}
    ...
    .pill small { display:block; font-weight:600; opacity:0.95; font-size:13px; }

    /* book list */
    .book-list{display:flex;flex-direction:column;gap:8px}
    .book-card{display:flex;justify-content:space-between;align-items:center;padding:12px;border-radius:10px;background:var(--glass);border:1px solid #f1f5f9;}
    .no-data{padding:12px;text-align:center;color:var(--muted) }
    .muted{color:var(--muted);font-size:13px}

    @media (max-width:880px){
      .app{grid-template-columns:1fr;}
      .loan-pills{justify-content:center}
      .pill{min-width:100px;padding:8px 18px}
    }
  </style>
</head>
<body>
  ...
  <div class="header">
      <div class="welcome">Welcome, <%= loggedIn.getUserName() %></div>
  ...
  <div class="app" role="application" aria-label="Library Dashboard">
    ...
    <section class="panel" aria-labelledby="listHeading">
      ...
      <div class="loan-pills" role="list" aria-label="Books currently on loan">
        <%
           // show up to 3 pills. if empty, show nothing (so no separate left block)
